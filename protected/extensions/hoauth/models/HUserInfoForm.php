...
class HUserInfoForm extends CFormModel
{
     ...
     * @access public
     */
    public function __construct($model, $emailAtt, $nameAtt, $scenario = 'both')
    {
        if (empty($emailAtt))
    ...
    public function attributeLabels()
    {
        return array(
            ...
            'login' => HOAuthAction::t('Nickname'),
            'pwd' => HOAuthAction::t('Password'),
        );
    }
    ...
    public function validateUser()
    {
        ...
        if (Users::model()->exists('email=:email', array(':email' => $this->email)))
            $this->addError('email', Yii::t('translations', 'Данный email уже используется в системе'));
