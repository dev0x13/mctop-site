...
class HOAuthAction extends CAction
{
    ...
    protected function oAuth($provider)
    {
        try {
            ...
            if (!Yii::app()->user->isGuest) {
                ...
                if (!$oAuth->isBond || !$user) {
                    ...
                    //$user = UsersSocialLogins::model()->findUser(Socials::model()->findByName($provider)->id, $userProfile->identifier);
                    ...
                    if (!isset($user->birhtday))
                        $user->birthday = HUtils::getCurrentTimestamp();
                ...
                // sign user in
                if ($accessCode === 1) {
                    $identity = new UserIdentity($user->id, null);
        ...
        if (isset($user)) {
            if ($user->login == (md5($user->registered)))
                Yii::app()->controller->redirect('/users/settings');
            else
    ...
    protected function processUser($user, $userProfile)
    {
        ...
        if ($_GET['provider'] == 'Vkontakte' | $_GET['provider'] == 'Facebook')
        ...
        if ($user->isNewRecord) {
            ...
            $social_login->id_in_system = $userProfile->identifier;
            $social_login->system = Socials::model()->findByName($this->provider)->id;
            $social_login->user = $user->id;
            $social_login->save();*/
